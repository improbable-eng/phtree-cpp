...
#ifndef PHTREE_COMMON_PH_TREE_STATS_H
#define PHTREE_COMMON_PH_TREE_STATS_H
...
class PhTreeStats {
  public:
    std::string ToString() {
        ...
        s << "  avgPostLen = " << apl << " (" << (MAX_BIT_WIDTH - apl) << ")" << std::endl;
        return s.str();
    }
    ...
    double GetAvgPostlen() {
        size_t total = 0;
        size_t num_entry = 0;
        for (bit_width_t i = 0; i < MAX_BIT_WIDTH; i++) {
            total += (MAX_BIT_WIDTH - i) * q_n_post_fix_n_[i];
  ...
  public:
    ...
    std::vector<size_t> q_n_post_fix_n_ =
        std::vector(MAX_BIT_WIDTH, (size_t)0);  // filled with  x[current_depth] = nPost;
    std::vector<size_t> infix_hist_ = std::vector(MAX_BIT_WIDTH, (size_t)0);       // prefix len
    std::vector<size_t> node_depth_hist_ = std::vector(MAX_BIT_WIDTH, (size_t)0);  // prefix len
    std::vector<size_t> node_size_log_hist_ = std::vector(32, (size_t)0);  // log (num_entries)
};

}  // namespace improbable::phtree
#endif  // PHTREE_COMMON_PH_TREE_STATS_H
