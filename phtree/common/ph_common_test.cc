...
TEST(PhTreeCommonTest, NumberOfDivergingBits) {
    ...
    scalar_t l_min = std::numeric_limits<scalar_t>::lowest();
    scalar_t l_max = std::numeric_limits<scalar_t>::max();

    bit_mask_t x = NumberOfDivergingBits(PhPoint<2>({l1, l1}), PhPoint<2>({l2, l2}));
    ASSERT_EQ(64, x);
