...
#include "../common/ph_tree_debug_helper.h"
#include "node.h"
...
class DebugHelperV16 : public PhTreeDebugHelper::DebugHelper {
  public:
    DebugHelperV16(const Node<DIM, T>& root, size_t size) : root_{root}, size_{size} {}
    ...
    [[nodiscard]] std::string ToString(
        ...
        case Enum::entries:
            ...
            break;
        case Enum::tree:
            ToStringTree(os, 0, root_, PhPoint<DIM>(), true);
            break;
        }
        return os.str();
    }
  ...
  private:
    void ToStringPlain(std::ostringstream& os, const Node<DIM, T>& node) const {
        for (auto& it : node.Entries()) {
            const PhEntry<DIM, T>& o = it.second;
            // inner node?
            if (o.IsNode()) {
    ...
    void ToStringTree(
        ...
        if (node.GetInfixLen() > 0) {
            bit_mask_t mask = MAX_MASK << node.GetInfixLen();
            mask = ~mask;
            mask <<= node.GetPostfixLen() + 1;
            for (dimension_t i = 0; i < DIM; ++i) {
                sb << ToBinary(prefix[i] & mask) << ",";
            }
        }
