...
function bazelLintTest() {
  ...
  CLANG_FORMAT="$(clangFormatLocation)" runBazel build --config lint --output_groups=clang_format_test -- $(generateBuildTargetString)
}

function bazelLintFix() {
  # Use bazel to create patch files for all eligible source files.
  CLANG_FORMAT="$(clangFormatLocation)" runBazel build --config lint --output_groups=clang_format_patches_only -- $(generateBuildTargetString)

  # Find bazel-bin prefix.
  BAZEL_BIN=$(runBazel info bazel-bin)
  # I.e. on Linux, this is `bazel-out/k8-gcc-opt/bin`.
  PREFIX=${BAZEL_BIN#$(runBazel info execution_root)/}
  ...
  CLANG_FORMAT="$(clangFormatLocation)" runBazel aquery --config lint --include_aspects --output_groups clang_format_patches_only "mnemonic(\"CreatePatch\", $(generateAqueryTargetString))" --output textproto \
